...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, n_channels=3, widen_factor=1, dropRate=0.0):
        ...
        self.conv1 = nn.Conv2d(self.input_channels, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        # 1st block
...
class MultiHeadWideResNet(nn.Module):
    def __init__(self, depth, num_classes_list, n_channels=3, widen_factor=1, dropRate=0.0):
        super(MultiHeadWideResNet, self).__init__()
        nChannels = [16, 16*widen_factor, 32*widen_factor, 64*widen_factor]
        # nChannels = [16, 16*widen_factor, 32*widen_factor, 32*widen_factor]
        ...
        for num_classes in num_classes_list:
            heads.append(nn.Linear(nChannels[3], num_classes))
